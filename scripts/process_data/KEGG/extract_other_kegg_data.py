...
class GetKeggOtherData(object):
    """This class is used to download KEGG data that is not associated with organisms and viruses"""
    ...
    def link_compound_to_pathway(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_pathway_id','kegg_compound_id']
    ...
    def link_compound_to_module(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_module_id','kegg_compound_id']
            else:
    ...
    def link_compound_to_glycan(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_glycan_id','kegg_compound_id']
            else:
    ...
    def link_compound_to_reaction(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_reaction_id','kegg_compound_id']
            else:
    ...
    def link_compound_to_enzyme(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_enzyme_id','kegg_compound_id']
            else:
    ...
    def link_compound_to_network(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_network_id','kegg_compound_id']
            else:
    ...
    def link_compound_to_disease(self, out_loc=None):
        if out_loc is None:
            self.logger.error("Please specify the parameters 'out_loc'")
            return 1
        else:
            if not os.path.exists(out_loc):
                os.makedirs(out_loc)
        link = self.get_request_link(operation='link',obj1='compound',obj2='disease')
        if link is not None:
            r = requests.get(link)
            if r.status_code == 200:
                self.logger.info(f"Successuflly link compounds to diseases from {link}")
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_disease_id','kegg_compound_id']
            else:
                self.logger.error(f"Fail to link compounds to diseases from {link}")
                return 0
            table.to_csv(os.path.join(out_loc,f"link_compound_to_disease.txt"), sep='\t', index=None)
            return 1
        else:
            self.logger.error(f"There are something wrong to get request link")
    ...
    def link_compound_to_drug(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_drug_id','kegg_compound_id']
            else:
    ...
    def link_compound_to_gn(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_gn_id','kegg_compound_id']
            else:
    ...
    def link_pathway_to_module(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_module_id','kegg_pathway_id']
                table['kegg_pathway_id'] = table['kegg_pathway_id'].str.replace('path:[a-z]*','path:map',regex=True)
                table = table.drop_duplicates().reset_index(drop=True)
            else:
    ...
    def link_pathway_to_glycan(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_glycan_id','kegg_pathway_id']
                table['kegg_pathway_id'] = table['kegg_pathway_id'].str.replace('path:[a-z]*','path:map',regex=True)
                table = table.drop_duplicates().reset_index(drop=True)
            else:
    ...
    def link_pathway_to_reaction(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_reaction_id','kegg_pathway_id']
                table['kegg_pathway_id'] = table['kegg_pathway_id'].str.replace('path:[a-z]*','path:map',regex=True)
                table = table.drop_duplicates().reset_index(drop=True)
            else:
    ...
    def link_pathway_to_enzyme(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_enzyme_id','kegg_pathway_id']
                table['kegg_pathway_id'] = table['kegg_pathway_id'].str.replace('path:[a-z]*','path:map',regex=True)
                table = table.drop_duplicates().reset_index(drop=True)
            else:
    ...
    def link_pathway_to_network(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_network_id','kegg_pathway_id']
                table['kegg_pathway_id'] = table['kegg_pathway_id'].str.replace('path:[a-z]*','path:map',regex=True)
                table = table.drop_duplicates().reset_index(drop=True)
            else:
    ...
    def link_pathway_to_ko(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_ko_id','kegg_pathway_id']
                table['kegg_pathway_id'] = table['kegg_pathway_id'].str.replace('path:[a-z]*','path:map',regex=True)
                table = table.drop_duplicates().reset_index(drop=True)
            else:
    ...
    def link_pathway_to_disease(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_disease_id','kegg_pathway_id']
                table['kegg_pathway_id'] = table['kegg_pathway_id'].str.replace('path:[a-z]*','path:map',regex=True)
                table = table.drop_duplicates().reset_index(drop=True)
            else:
    ...
    def link_pathway_to_drug(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_drug_id','kegg_pathway_id']
                table['kegg_pathway_id'] = table['kegg_pathway_id'].str.replace('path:[a-z]*','path:map',regex=True)
                table = table.drop_duplicates().reset_index(drop=True)
            else:
    ...
    def link_pathway_to_gn(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_gn_id','kegg_pathway_id']
                table['kegg_pathway_id'] = table['kegg_pathway_id'].str.replace('path:[a-z]*','path:map',regex=True)
                table = table.drop_duplicates().reset_index(drop=True)
            else:
    ...
    def link_pathway_to_rclass(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_rclass_id','kegg_pathway_id']
                table['kegg_pathway_id'] = table['kegg_pathway_id'].str.replace('path:[a-z]*','path:map',regex=True)
                table = table.drop_duplicates().reset_index(drop=True)
            else:
    ...
    def link_module_to_glycan(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_glycan_id','kegg_module_id']
            else:
    ...
    def link_module_to_reaction(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_reaction_id','kegg_module_id']
            else:
    ...
    def link_module_to_enzyme(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_enzyme_id','kegg_module_id']
            else:
    ...
    def link_module_to_ko(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_ko_id','kegg_module_id']
            else:
    ...
    def link_module_to_disease(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_disease_id','kegg_module_id']
            else:
    ...
    def link_module_to_gn(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_gn_id','kegg_module_id']
    ...
    def link_glycan_to_reaction(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_reaction_id','kegg_glycan_id']
            else:
    ...
    def link_glycan_to_enzyme(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_enzyme_id','kegg_glycan_id']
            else:
    ...
    def link_reaction_to_enzyme(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_enzyme_id','kegg_reaction_id']
            else:
    ...
    def link_reaction_to_ko(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_ko_id','kegg_reaction_id']
            else:
    ...
    def link_reaction_to_rclass(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_rclass_id','kegg_reaction_id']
            else:
    ...
    def link_enzyme_to_ko(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_ko_id','kegg_enzyme_id']
            else:
    ...
    def link_enzyme_to_rclass(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_rclass_id','kegg_enzyme_id']
            else:
    ...
    def link_network_to_ko(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_ko_id','kegg_network_id']
            else:
    ...
    def link_network_to_disease(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_disease_id','kegg_network_id']
            else:
    ...
    def link_ko_to_disease(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_disease_id','kegg_ko_id']
            else:
    ...
    def link_ko_to_drug(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_drug_id','kegg_ko_id']
            else:
    ...
    def link_ko_to_rclass(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_rclass_id','kegg_ko_id']
            else:
    ...
    def link_disease_to_drug(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_drug_id','kegg_disease_id']
            else:
    ...
    def link_disease_to_gn(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_gn_id','kegg_disease_id']
            else:
    ...
    def link_drug_to_dgroup(self, out_loc=None):
        ...
        if link is not None:
            ...
            if r.status_code == 200:
                ...
                table = pd.DataFrame([x.split('\t') for x in r.text.split('\n') if x.split('\t')[0]])
                table.columns = ['kegg_dgroup_id','kegg_drug_id']
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--log_name", type=str, required=False, default="extract_other_kegg_data.log", help="log file name")
    ...
    get_kegg_data = GetKeggOtherData()
    ## download kegg compound information
    get_kegg_data.download_kegg_compound(args.outdir)
    ## download kegg pathway information
    get_kegg_data.download_kegg_pathway(args.outdir)
    ## download kegg module information
    get_kegg_data.download_kegg_module(args.outdir)
    ## download kegg glycan information
    get_kegg_data.download_kegg_glycan(args.outdir)
    ## download kegg reaction information
    get_kegg_data.download_kegg_reaction(args.outdir)
    ## download kegg enzyme information
    get_kegg_data.download_kegg_enzyme(args.outdir)
    ## download kegg network information
    get_kegg_data.download_kegg_network(args.outdir)
    ## download kegg ko information
    get_kegg_data.download_kegg_ko(args.outdir)
    ## download kegg disease information
    get_kegg_data.download_kegg_disease(args.outdir)
    ## download kegg drug information
    get_kegg_data.download_kegg_drug(args.outdir)
    ## download kegg genome information
    get_kegg_data.download_kegg_gn(args.outdir)
    ## download kegg reaction class information
    get_kegg_data.download_kegg_rclass(args.outdir)
    ## download kegg drug class information
    get_kegg_data.download_kegg_dgroup(args.outdir)

    ## link compound information to pathway information
    get_kegg_data.link_compound_to_pathway(args.outdir)
    ## link compound information to module information
    get_kegg_data.link_compound_to_module(args.outdir)
    ## link compound information to glycan information
    get_kegg_data.link_compound_to_glycan(args.outdir)
    ## link compound information to reaction information
    get_kegg_data.link_compound_to_reaction(args.outdir)
    ## link compound information to enzyme information
    get_kegg_data.link_compound_to_enzyme(args.outdir)
    ## link compound information to network information
    get_kegg_data.link_compound_to_network(args.outdir)
    ## link compound information to disease information
    get_kegg_data.link_compound_to_disease(args.outdir)
    ## link compound information to drug information
    get_kegg_data.link_compound_to_drug(args.outdir)
    ## link compound information to genome information
    get_kegg_data.link_compound_to_gn(args.outdir)
    ## link pathway information to module information
    get_kegg_data.link_pathway_to_module(args.outdir)
    ## link pathway information to glycan information
    get_kegg_data.link_pathway_to_glycan(args.outdir)
    ## link pathway information to reaction information
    get_kegg_data.link_pathway_to_reaction(args.outdir)
    ## link pathway information to enzyme information
    get_kegg_data.link_pathway_to_enzyme(args.outdir)
    ## link pathway information to network information
    get_kegg_data.link_pathway_to_network(args.outdir)
    ## link pathway information to ko information
    get_kegg_data.link_pathway_to_ko(args.outdir)
    ## link pathway information to disease information
    get_kegg_data.link_pathway_to_disease(args.outdir)
    ## link pathway information to drug information
    get_kegg_data.link_pathway_to_drug(args.outdir)
    ## link pathway information to genome information
    get_kegg_data.link_pathway_to_gn(args.outdir)
    ## link pathway information to reaction class information
    get_kegg_data.link_pathway_to_rclass(args.outdir)
    ## link module information to glycan information
    get_kegg_data.link_module_to_glycan(args.outdir)
    ## link module information to reaction information
    get_kegg_data.link_module_to_reaction(args.outdir)
    ## link module information to enzyme information
    get_kegg_data.link_module_to_enzyme(args.outdir)
    ## link module information to ko information
    get_kegg_data.link_module_to_ko(args.outdir)
    ## link module information to disease information
    get_kegg_data.link_module_to_disease(args.outdir)
    ## link module information to genome information
    get_kegg_data.link_module_to_gn(args.outdir)
    ## link glycan information to reaction information
    get_kegg_data.link_glycan_to_reaction(args.outdir)
    ## link glycan information to reaction information
    get_kegg_data.link_glycan_to_enzyme(args.outdir)
    ## link reaction information to enzyme information
    get_kegg_data.link_reaction_to_enzyme(args.outdir)
    ## link reaction information to ko information
    get_kegg_data.link_reaction_to_ko(args.outdir)
    ## link reaction information to rclass information
    get_kegg_data.link_reaction_to_rclass(args.outdir)
    ## link enzyme information to ko information
    get_kegg_data.link_enzyme_to_ko(args.outdir)
    ## link enzyme information to rclass information
    get_kegg_data.link_enzyme_to_rclass(args.outdir)
    ## link network information to ko information
    get_kegg_data.link_network_to_ko(args.outdir)
    ## link network information to disease information
    get_kegg_data.link_network_to_disease(args.outdir)
    ## link ko information to disease information
    get_kegg_data.link_ko_to_disease(args.outdir)
    ## link ko information to drug information
    get_kegg_data.link_ko_to_drug(args.outdir)
    ## link ko information to reaction class information
    get_kegg_data.link_ko_to_rclass(args.outdir)
    ## link disease information to drug information
    get_kegg_data.link_disease_to_drug(args.outdir)
    ## link disease information to genome information
    get_kegg_data.link_disease_to_gn(args.outdir)
    ## link drug information to drug group information
    get_kegg_data.link_drug_to_dgroup(args.outdir)
