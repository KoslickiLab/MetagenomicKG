...
class ModelController(object):
    ...
    def __init__(self, args):
        ...
        self.summary_table_test_cv = os.path.join(self.output_dir, self.experiment_name, f"performance_summary_{self.cv_num}_cv_test.tsv")

    def run(self):
        """
        Run model training
        """
        comments(f"Train Model with {self.cv_num}Fold Cross Validation", logger=self.logger)
    ...
    def run_cv_training(self):
        ...
        data_obj = ProcessedDataset(processdata_dir, self.data_path, self.ratio, self.random_state, self.cv_num, self.logger)
        
        # load processed data
        kfold_index = data_obj.load_kfold_index()
        holdout_index = data_obj.load_holdout_index()
        ...
        X_holdout, y_holdout = holdout_index
        X_holdout = torch.tensor(X_holdout, dtype=torch.int32)
        y_holdout = torch.tensor(y_holdout, dtype=torch.int32)
        sorted_indices = torch.argsort(X_holdout)
        holdout_sorted_X = X_holdout[sorted_indices]
        holdout_sorted_y = y_holdout[sorted_indices]
        if self.args.use_gpu:
            X_holdout = X_holdout.to(self.device)
            holdout_dataloader = DataLoader(mkg, X_holdout, eval_step_sampler, device=self.device, batch_size=self.eval_batch_size, \
                shuffle=True, drop_last=False, num_workers=0)
        else:
            holdout_dataloader = DataLoader(mkg, X_holdout, eval_step_sampler, batch_size=self.eval_batch_size, \
                shuffle=True, drop_last=False, num_workers=self.num_workers)

        for fold in range(self.cv_num):
            self.logger.info(f"Starting CV Iteration {fold+1}/{self.cv_num}")
            ...
            ## set up tensorboard
            writer = SummaryWriter(log_dir=os.path.join(self.output_dir, self.experiment_name, f'{self.args.cv_num}cv', f'{fold+1}fold', 'tensorboard_runs'))

            ## Create data loaders
            (X_train, y_train), (X_val, y_val), (X_test, y_test) = kfold_index[fold]
            # process train data
            ...
            if self.args.use_gpu:
                ...
                train_dataloader = DataLoader(mkg, X_train, train_step_sampler, device=self.device, batch_size=self.train_batch_size, \
                                    shuffle=True, drop_last=False, num_workers=0)
            else:
                train_dataloader = DataLoader(mkg, X_train, train_step_sampler, batch_size=self.train_batch_size, \
                    shuffle=True, drop_last=False, num_workers=self.num_workers)
            ...
            data = {"train_dataloader": train_dataloader,
                    ...
                    "holdout_sorted_X": holdout_sorted_X,
                    "holdout_sorted_y": holdout_sorted_y
                    }
            ...
            self.logger.info(f"Saving best model and data for {fold+1} CV iteration")
            self.save_model(train_obj.best_model_state, train_obj.best_model_name, fold+1)
            self.save_data(data, 'data.pkl', fold+1)

            ## evaluate model
            ## load best model
            model_state = self.load_best_model(train_obj.best_model_name, fold+1)
            model = MyModel(self.num_layers, mkg.ndata['feat'].shape[1], self.emb_size, 1, self.dropout_p)
            ...
            self.plot_performance_metrics(precision, recall, fpr, tpr, fold+1, 'test')
            holdout_obj = Tester(self.args, model, data, metrics_obj, mode='holdout')
            holdout_acc, holdout_auroc, holdout_ap, holdout_f1score, holdout_precision, holdout_recall, holdout_fpr, holdout_tpr, holdout_pos_acc, holdout_neg_acc = holdout_obj.run()
            self.update_model_stats(holdout_acc, holdout_pos_acc, holdout_neg_acc, holdout_auroc, holdout_ap, holdout_f1score, 'holdout')
            self.plot_performance_metrics(holdout_precision, holdout_recall, holdout_fpr, holdout_tpr, fold+1, 'holdout')
            self.logger.info(f"Finished CV Iteration {fold+1}/{self.cv_num}")
            

    def save_model(self, model_state, model_name, fold):
        """
        Save the model parameters
        Args:
            model_state (model.state_dict): model parameters
            model_name (str): model filename name
            fold (int): cross validation iteration
        Returns: None
        """
        save_dir = os.path.join(self.output_dir, self.experiment_name, f'{self.args.cv_num}cv', f'{fold}fold')
    ...
    def save_data(self, data, data_name, fold):
        """
        Save the data
        Args:
    ...
    def load_best_model(self, model_name, fold):
        """
        ...
        # load model
        model_state = torch.load(os.path.join(self.output_dir, self.experiment_name, f'{self.args.cv_num}cv', f'{fold}fold', model_name))
        return model_state
    ...
    def save_model_stats(self):
        ...
        df.index = [f'fold{i+1}' for i in range(self.cv_num)]
        df.to_csv(self.summary_table_test_cv, sep='\t', index=True, header=True)


    def plot_performance_metrics(self, precision, recall, fpr, tpr, fold, type):
        """
        ...
        Returns: None
        """
        pr_path = os.path.join(self.output_dir, self.experiment_name, f'{self.args.cv_num}cv', f'{fold}fold', f"{type}_precision-recall_curve.png")
        p = sns.lineplot(x=recall, y=precision)
        p.set(xlabel="Recall", ylabel="Precision")
        plt.savefig(pr_path, bbox_inches='tight')
        plt.clf()

        roc_path = os.path.join(self.output_dir, self.experiment_name, f'{self.args.cv_num}cv', f'{fold}fold', f"{type}_roc_curve.png")
